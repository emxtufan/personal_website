...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const mapped = palette.filter(Boolean).slice(0, MAX_COLORS).map(toVec3);
    for (let i = 0; i < MAX_COLORS; i += 1) {
      const vec = (material.uniforms.uColors.value as THREE.Vector3[])[i];
      if (i < mapped.length) vec.copy(mapped[i]);
      else vec.set(0, 0, 0);
    }

    material.uniforms.uColorCount.value = mapped.length;
